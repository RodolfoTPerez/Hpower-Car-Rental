...
const calculate = async (req, res) => {
  try {
    const { vehicleId, startDate, endDate, locationId } = req.query
    if (!vehicleId || !startDate || !endDate)
      return error(res, 'vehicleId, startDate y endDate son requeridos', 400)

    // Intentar usar nueva RPC con location
    const { data, error: err } = await supabase
      .rpc('calculate_reservation_price_with_location', {
    ...
    // Si la nueva RPC no existe, usar la antigua como fallback
    if (err && err.message.includes('function') && err.message.includes('does not exist')) {
      console.log(' Nueva RPC no encontrada, usando RPC antigua como fallback');
      ...
      if (fallbackErr) return error(res, 'Error al calcular precio', 500, fallbackErr)
      return success(res, fallbackData[0])
    }
